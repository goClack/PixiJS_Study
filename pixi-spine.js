                    ...
                    while (true) {
                        ...
                        if (line.length == 0)
                            ...
                            textureLoader(line, function (texture) {
                                /* network error handling, 20181130 by vince */
                                if (!texture) {
                                    _this.error = true;
...
(function (pixi_spine) {
    ...
    function atlasParser() {
        return function (resource, next) {
            ...
            var loader = this;
            var createSkeletonWithRawAtlas = function (rawData, retryCount) {
                new pixi_spine.core.TextureAtlas(rawData, adapter, function (spineAtlas) {
                    /* network error handling, 20181130 by vince */
                    if (spineAtlas.error) {
                        retryCount = retryCount || 0;
	                    if (retryCount < 4) {
		                    console.log('[spine] load atlas: retry=' + retryCount);
	                        setTimeout(function () {
	                            createSkeletonWithRawAtlas(rawData, retryCount + 1);
	                        }, 2500);
                        } else {
                            console.error('[spine] load failed.');
                            loader.emit('lost_connection');
                        }
                        return;
                    }
            ...
            if (resource.metadata && resource.metadata.atlasRawData) {
                ...
                this.add(resource.name + '_atlas', atlasPath, atlasOptions, cb);
                this.resources[resource.name + '_atlas'].__options = atlasOptions;
                this.resources[resource.name + '_atlas'].__callback = cb;
            }
        };
    }
    pixi_spine.atlasParser = atlasParser;
    pixi_spine.atlasParserInstance = atlasParser();
    ...
    PIXI.loader.use(pixi_spine.atlasParserInstance);
})(pixi_spine || (pixi_spine = {}));
